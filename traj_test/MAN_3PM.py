...
from dynamics.MANAug3PM import MANAug3PM
import os
import numpy as np
...
def traj_test(model, initial_state, dynamics, dt = 0.0025, tMax = 1.0, tMin = 0):
    ...
    for k in tqdm(range(int((tMax-tMin)/dt)), desc='Trajectory Propagation', position=pbar_pos, leave=False):
        ...
        if (traj_time <= dt):
            ...
            print(state_trajs[:, k+1])
            print("Terminal_Cost", dynamics.l_x(state_trajs[:, k+1]))
        pbar_pos +=1
        # print()

    traj = state_trajs[0].T
    x1, y1, x2, y2, x3, y3, gx_1, gy_1, gx_2, gy_2, gx_3, gy_3, z = traj
    ...
    # plt.savefig("MAN9D/z_time_plot.png",dpi=1200) 
    ...
    plt.savefig("plots/MAN/traj_plot_3.png",dpi=1200)    
    # plt.show()    
...
if __name__ =="__main__":
    ...
    model_path = os.path.join('runs/MANAug3PM_VDR', 'training', 'checkpoints', 'model_final.pth')
    ...
    times = torch.Tensor([1.0])
                        #    x1   y1   x2  y2   x3   y3   gx1  gy1  gx2  gy2  gx3  gy3   z
    states = torch.Tensor(([0.0, 0.0, 0.5, 0.3, 0.5,-0.3,-0.3, 0.0, 0.51,-0.3,0.49, 0.3, 0.0],
                            )).to('cuda')
    
    for i in range(len(states)): #range(0,1): #
        act_state = states[i][0:12]
        # print(act_state)
