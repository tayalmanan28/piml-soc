...
def traj_test(model, initial_state, dynamics, traj_num, dt = 0.0025, tMax = 2.0, tMin = 0):
    ...
    num_vehicles = 4  # Update this if your vehicle count changes
...
if __name__ =="__main__":
    ...
    model_path = os.path.join('runs/MANAug4PM', 'training', 'checkpoints', 'model_final.pth')
    model.load_state_dict(torch.load(model_path)['model'])
    model.cuda()
    times = torch.Tensor([1.0])
                        #    x1   y1   x2    y2  x3   y3    x4  y4   gx1  gy1  gx2  gy2  gx3  gy3  gx4  gy4   z
    states = torch.Tensor(([0.2, 0.8,-0.2,  0.8, 0.8, 0.2, 1.8, 0.6, 0.2, 0.8,-0.2, 0.8, 0.8, 0.2, 0.8, 0.6, 0.0],
                            )).to('cuda')
