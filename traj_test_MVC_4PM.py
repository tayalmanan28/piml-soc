...
from dynamics.MVCAug4PM import MVCAug4PM
import os
import numpy as np
...
from anim4V import animate_trajectories
...
def traj_test(model, initial_state, dynamics, dt = 0.0025, tMax = 1.0, tMin = 0):
    ...
    x1, y1, x2, y2, x3, y3, x4, y4, gx_1, gy_1, gx_2, gy_2, gx_3, gy_3, gx_4, gy_4, z = traj
    ...
    animate_trajectories(x1, y1, x2, y2, x3, y3, x4, y4, gx_1, gy_1, gx_2, gy_2, gx_3, gy_3, gx_4, gy_4)
    ...
    plt.savefig("plots/MVC/traj_plot_4.png",dpi=1200)    
    # plt.show()    
...
if __name__ =="__main__":
    ...
    model_path = os.path.join('runs/MVCAug4PM_VDR', 'training', 'checkpoints', 'model_final.pth')
    ...
    times = torch.Tensor([1.0])
                        #    x1   y1   x2    y2  x3   y3    x4  y4   gx1  gy1  gx2  gy2  gx3  gy3  gx4  gy4   z
    states = torch.Tensor(([0.2, -0.8,-0.2,-0.8,-0.8, 0.2,-0.8,-0.2, 0.2, 0.8,-0.2, 0.8, 0.8, 0.2, 0.8,-0.2, 0.0],
                            )).to('cuda')
    
    for i in range(len(states)): #range(0,1): #
        act_state = states[i][0:16]
        # print(act_state)
