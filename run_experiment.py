...
if (mode == 'all') or (mode == 'train'):
    ...
    p.add_argument('--num_switch_samples', type=int, default=0,
                   required=False, help='Number of samples inside the switch set')
    ...
    p.add_argument('--val_z_resolution', type=int, default=3,
...
if (mode == 'all') or (mode == 'test'):
    ...
    p.add_argument('--data_step', type=str, default='run_basic_recovery', choices=['plot_violations', 'run_basic_recovery', 'plot_basic_recovery',
                   'collect_samples', 'train_binner', 'run_binned_recovery', 'plot_binned_recovery', 'plot_cost_function', 'run_robust_recovery', 'run_perf_recovery'], help='The data processing step to run')
...
dataset = dataio.ReachabilityDataset(
    ...
    num_src_samples=orig_opt.num_src_samples, num_target_samples=orig_opt.num_target_samples, num_switch_samples=orig_opt.num_switch_samples)
...
if (mode == 'all') or (mode == 'train'):
    ...
    experiment.train(
        ...
        max_CSL_epochs=orig_opt.max_CSL_epochs, CSL_loss_weight=orig_opt.CSL_loss_weight, CSL_batch_size=orig_opt.CSL_batch_size, viscosity_coef=orig_opt.viscosity_coef)
