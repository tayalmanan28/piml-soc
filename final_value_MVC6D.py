...
def opt_value_func_mesh(model, dynamics, traj_time, x1, y1, x2, y2, th1, th2, z_range, resolution, num_z, plot_flag= False):
    policy = model.eval()
        ...
        for j in range(resolution):
            ...
            for z in z_list:
                traj_coord = torch.cat(
                    (traj_time.to('cuda'), torch.Tensor([x1, y1, x2, y2, th1, th2]).to('cuda'), z.unsqueeze(0).to('cuda')), dim=-1
                ).reshape(1, 8)
                with torch.no_grad():
                    V_hat = policy(
