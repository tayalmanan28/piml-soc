import torch
from dynamics.MVC9DAug import MVC9DAug
import os
...
def opt_value_func_mesh(model, dynamics, traj_time, x1, y1, x2, y2, x3, y3, th1, th2, th3, z_range, resolution, num_z, plot_flag= False):
    policy = model.eval()
        ...
        for j in range(resolution):
            ...
            for z in z_list:
                traj_coord = torch.cat(
                    (traj_time.to('cuda'), torch.Tensor([x1, y1, x2, y2, x3, y3, th1, th2, th3]).to('cuda'), z.unsqueeze(0).to('cuda')), dim=-1
                ).reshape(1, 11)
                with torch.no_grad():
                    V_hat = policy(
...
if __name__ == "__main__":
    ...
    model_path = os.path.join('runs/MVC9DAug_VDR', 'training', 'checkpoints', 'model_final.pth')
    model.load_state_dict(torch.load(model_path)['model'])
    model.cuda()
    states = np.array(([-0.3, 0, -1, 0.3, 1, 0.3, 0, 0, 0],
