...
class MANAug3PM(Dynamics):
    def __init__(self):
        ...
        self.z_var = 2.2
        super().__init__(
            loss_type='brt_aug_hjivi', set_mode=self.set_mode, state_dim=13, input_dim=14, control_dim=6, disturbance_dim=0,
            state_mean=[
                0, 0,
                0, 0,
                0, 0,
    ...
    def dsdt(self, state, control, disturbance):
        ...
        dsdt[..., 4] = control[..., 4]
        dsdt[..., 5] = control[..., 5]
        dsdt[..., 12] = -self.l_x(state)
        return dsdt
    
    def l_x(self, state):
        dist1 = state[..., 0:2]*1.0
        dist1[..., 0] = dist1[..., 0]- state[..., 6]
        dist1[..., 1] = dist1[..., 1]- state[..., 7]
        distance_1 = torch.norm(dist1[..., 0:2], dim=-1) - self.goalR

        dist2 = state[..., 2:4]*1.0
        dist2[..., 0] = dist2[..., 0]- state[..., 8]
        dist2[..., 1] = dist2[..., 1]- state[..., 9]
        distance_2 = torch.norm(dist2[..., 0:2], dim=-1) - self.goalR

        dist3 = state[..., 4:6]*1.0
        dist3[..., 0] = dist3[..., 0]- state[..., 10]
        dist3[..., 1] = dist3[..., 1]- state[..., 11]
    ...
    def boundary_fn(self, state):
        return torch.maximum(self.avoid_fn(state), self.l_x(state) - state[...,12])
    ...
    def hamiltonian(self, state, dvds):
        ...
        ham = ham - dvds[..., 12]*self.l_x(state)
        return ham

    def optimal_control(self, state, dvds):
        return -1.0*self.velocity*torch.sign(dvds[..., [0, 1, 2, 3, 4, 5]])
