...
def animate_trajectory(x, y, traj_num):
    """
    ...
    """
    # Load the boat image
    boat_img = mpimg.imread('plots/Boat2D/boat.png')
    ...
    # Plot the trajectory
    ax.plot(x, y, '--', alpha=0.5, label='Trajectory')

    # Add circles (as in your original code)
    ax.add_patch(Circle((-0.5, 0.5), 0.4, facecolor='#6B6B6B', alpha=1))
    ax.add_patch(Circle((-1.0, -1.2), 0.5, facecolor='#6B6B6B', alpha=1))
    ax.add_patch(Circle((1.5, 0), 0.025, facecolor='green', alpha=1))
    ...
    ani.save(f'plots/Boat2D/animation_{traj_num}.gif', writer='pillow', fps=30)
