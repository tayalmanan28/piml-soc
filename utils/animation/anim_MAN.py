...
def animate_trajectories(vehicle_trajectories, goal_positions, traj_number, interval=10, marker_size=0.25, save_interval=200, background_color='#E3E3E3'):
    ...
    def update(frame):
        ...
        if frame % save_interval == 0:
            plt.savefig(f'plots/MAN/MAN_{num_vehicles}_traj_{traj_number}_frame_{frame}.png', facecolor=fig.get_facecolor())
    ...
    ani.save(f'plots/MAN/MAN_{num_vehicles}_traj_{traj_number}_animation.gif', fps=30, writer='pillow', savefig_kwargs={'facecolor': background_color})
