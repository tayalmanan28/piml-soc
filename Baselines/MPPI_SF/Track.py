...
if __name__ =="__main__":
    ...
    for i in range(rollout_points.shape[0]):
        ...
        rollout_cost, trajectory, _ = run_batch_mppi(device, x0, dynamics, mppi_cost_func, running_cost, obs_cost, solve_qp, total_time_steps, num_rollouts, planning_horizon, dt, goal, obstacles, softmax_lambda, u_dim=2, norm_flag =False, clip_flag =True)
        ...
        plt.savefig("plots/Track/mppi_sf.png",dpi=1200) 
        pd.DataFrame(x).to_csv('plots/Track/mppi_sf_traj_x'+str(i)+'.csv', index=False, header=["x"])
        pd.DataFrame(y).to_csv('plots/Track/mppi_sf_traj_y'+str(i)+'.csv', index=False, header=["y"])

    rollout_costs = rollout_costs.cpu()
    # Convert tensors to NumPy and save to CSV
    pd.DataFrame(rollout_costs.numpy()).to_csv('plots/Track/mppi_sf_costs.csv', index=False, header=["rollout_costs"])
