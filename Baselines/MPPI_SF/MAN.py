...
if __name__ =="__main__":
    ...
    # Load rollout points
    rollout_points = csv_to_tensor(file_path)[0:20].to(device).T
    ...
    for i in range(rollout_points.shape[0]):
        ...
        rollout_cost, trajectory, _ = run_batch_mppi(device, x0, dynamics, mppi_cost_func, running_cost, obs_cost, solve_qp, total_time_steps, num_rollouts, planning_horizon, dt, goal, obstacles, softmax_lambda, u_dim=10, norm_flag =True)
    ...
    pd.DataFrame(rollout_costs.numpy()).to_csv('plots/MAN/mppi_sf_costs.csv', index=False, header=["rollout_costs"])
