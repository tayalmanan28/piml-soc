...
def solve_qp(u_nom, x, obstacles=obstacles):
    u = cp.Variable((num_agents, dim))
    u_nom = u_nom.reshape(num_agents, dim)
    x = x.reshape(num_agents, dim)
    ...
    for i in range(num_agents):
        for j in range(i + 1, num_agents):
            ...
            constraints.append(A_ij @ u.flatten() + 1000*(np.linalg.norm(x[i] - x[j])**2 - (delta_c )**2)**3 >= 0.0)
