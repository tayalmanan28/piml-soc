...
def run_batch_mppi(device, x0, dynamics, mppi_cost_func, running_cost, obs_cost, solve_qp, total_time_steps, num_rollouts, planning_horizon, dt, goal, obs, softmax_lambda, u_dim=2, norm_flag =False, clip_flag =False):
    ...
    for step in range(0, total_time_steps):
        ...
        running_costs[step] = running_cost(torch.Tensor(current_x), goal, device).to('cpu').detach() + 1e4*obs_cost(torch.Tensor(current_x), obs, device).to('cpu').detach()#.numpy()#obs_distance*1e12
        ...
        # print(current_x, best_u[0, :].reshape(1, u_dim), obs)
        best_u = solve_qp(best_u[0, :].reshape(1, u_dim).to('cpu').detach(), current_x[:,0:10].to('cpu').detach())
        best_u = torch.Tensor(best_u.reshape(1, u_dim)).to('cuda')
