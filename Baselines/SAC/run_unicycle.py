...
model = SAC.load("Baselines/SAC/sac_unicycle")
...
initial_points = np.array(([-0.8, 0.5, 1.3, -0.65, -0.2, -0.2, -0.1, 0.0],
                                  [0.0, -0.5, 1.2, 0.0, 0.75, -0.4, -0.35, 0.35]))
...
for idx, initial_state in enumerate(initial_points):
    ...
    cost, x, y  = simulate_trajectory(initial_state)
    ...
    pd.DataFrame(x).to_csv('plots/Track/SAC_traj_x'+str(idx)+'.csv', index=False, header=["x"])
    pd.DataFrame(y).to_csv('plots/Track/SAC_traj_y'+str(idx)+'.csv', index=False, header=["y"])

# Save results to a CSV file
# results_df = pd.DataFrame(results, columns=["Cost",])
# results_df.to_csv("plots/Track/rollout_costs_sac.csv", index=False)
